...
using SitefinityMailServer.POP3.Log;
using SitefinityMailServer.POP3.Net;

namespace SitefinityMailServer.POP3.TCP
{    
    /// <summary>
    ...
    public class TCP_Server<T> : IDisposable where T : TCP_ServerSession,new()
    {
        ...
        public delegate void UserAuthenticatedEventHandler(object sender, Pop3SessionEventArgs e);
        public event UserAuthenticatedEventHandler UserAuthenticated;

        public delegate void UserQuitEventHandler(object sender, Pop3SessionEventArgs e);
        public event UserQuitEventHandler UserQuit;


        public void OnUserAuthenticated(POP3_Session session, Pop3SessionEventArgs args)
        {
            if (UserAuthenticated != null)
                UserAuthenticated(session, args);
        }

        public void OnUserQuit(POP3_Session session, Pop3SessionEventArgs args)
        {
            if (UserQuit != null)
                UserQuit(session, args);
        }
